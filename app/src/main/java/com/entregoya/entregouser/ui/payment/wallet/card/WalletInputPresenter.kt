...
class WalletInputPresenter : BaseMvpPresenter<WalletInputContract.View>(), WalletInputContract.Presenter {
    ...
    val mAddCardListener = object : AddCardRequest.AddCardListener {
        ...
        override fun onValidationError(fields: Array<FieldError>) {
            ...
            mView?.hideProgress()
            fields.forEach {
                val filed = WalletInputContract.FieldType.fromString(it.field)
    ...
    override fun saveCard(name: String, number: String, month: String, year: String, cvv: String) {
        ...
        if (name.isEmpty()) {
            mView?.setError(WalletInputContract.FieldType.NAME, R.string.error_required_field)
            return
        }
        if (number.isEmpty()) {
            mView?.setError(WalletInputContract.FieldType.NUMBER, R.string.error_required_field)
            return
        }
        if (month.isEmpty()) {
            mView?.setError(WalletInputContract.FieldType.MONTH, R.string.error_required_field)
            return
        }
        if (year.isEmpty()) {
            mView?.setError(WalletInputContract.FieldType.YEAR, R.string.error_required_field)
            return
        }
        if (cvv.isEmpty()) {
            mView?.setError(WalletInputContract.FieldType.CVV, R.string.error_required_field)
            return
        }

        if (number.isValidLong().not()) {
            mView?.setError(WalletInputContract.FieldType.NUMBER, R.string.error_invalid_value)
            return
        }
        if (month.isValidInt().not()) {
            mView?.setError(WalletInputContract.FieldType.MONTH, R.string.error_invalid_value)
            return
        }
        if (year.isValidInt().not()) {
            mView?.setError(WalletInputContract.FieldType.YEAR, R.string.error_invalid_value)
            return
        }
        if (cvv.isValidShort().not()) {
            mView?.setError(WalletInputContract.FieldType.CVV, R.string.error_invalid_value)
            return
        }
